...
Adds 5 columns to a filtered jobs DataFrame:
  stack_hits  int    Count of profile stack keywords in JD description.
  ...
  priority    str    HIGH | MOD | LOW | ?
                     ...
                     consumers (e.g. downstream CSV importers) regularly
...
To add stack keywords: edit prescreen.stack_keywords in config/profile.yaml.
"""
...
from comp_extract import add_comp_columns
...
from profile_loader import (  # noqa: E402
    comp_settings,
    prescreen_priority_limits,
    prescreen_stack_keywords,
)
...
def _score_stack(text) -> int:
    ...
    return sum(1 for kw in prescreen_stack_keywords() if kw in t)
...
def _compute_priority(stack_hits: int, yrs_req,
    ...
    limits = prescreen_priority_limits()

    # Hard blockers — NaN yrs_req passes (unstated ≠ required)
    if not _yrs_ok(yrs_req, limits['max_years_low']):
        return 'LOW'
    if domain == 'far':
        return 'LOW'

    # Strong signal
    if stack_hits >= 3 and _yrs_ok(yrs_req, limits['max_years_high']):
        return 'HIGH'

    # Moderate signal
    if stack_hits >= 1 and _yrs_ok(yrs_req, limits['max_years_mod']):
        return 'MOD'
...
def add_prescreen_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    jd_comp_min, jd_comp_max, jd_comp_mid, jd_comp_snippet, jd_comp_source,
    jd_comp_disclosed, gate2_at_145k, effective_comp_min, effective_comp_max.
